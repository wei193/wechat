...
//Dealwith 处理微信自动回复消息
func (wx *Wechat) Dealwith(req *STMsgRequest) (resp interface{}, err error) {
	if req.MsgType == Event {
		d, e := wx.eventFuncall(req)
		if d != nil {
			return d, e
		}
	}
	...
	return wx.getResult(req)
}

//查询处理结果
func (wx *Wechat) getResult(req *STMsgRequest) (resp interface{}, err error) {
	var auto STAutoReply
	autoRes, ok := wx.AutoReply[req.MsgType]
	if ok {
		switch req.MsgType {
		case Text:
			if _, ok = autoRes[req.Content]; ok {
				auto = autoRes[req.Content]
			} else { //模糊查询
				for _, v := range autoRes {
					if v.ReqEvent == "fuzzy" && strings.Index(req.Content, v.ReqContent) != -1 {
						auto = v
						break
					}
				}
		...
		case Event:
			...
			if req.EventKey == "" {
				...
				key = req.Event + "_" + req.EventKey
			}
			if _, ok = autoRes[key]; ok {
				auto = autoRes[key]
			}
		default:
			if _, ok = autoRes["default"]; ok {
				auto = autoRes["default"]
			}
		}
		return createResponse(req, auto)
	}

	autoRes, ok = wx.AutoReply["default"]
	if ok {
		auto := autoRes["default"]
		return createResponse(req, auto)
	}
...
//事件类消息回调函数处理
func (wx *Wechat) eventFuncall(req *STMsgRequest) (resp interface{}, err error) {
